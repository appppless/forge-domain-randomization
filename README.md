...
randomization to Isaac Sim / Omniverse. It scans the current stage, lets users
...
5. Add the absolute path to this repository's `extensions` folder.
6. Search for `FORGE Domain Randomization`.
7. Enable the `forge.domain_randomization` extension.

After the extension is enabled, the panel is available from:
